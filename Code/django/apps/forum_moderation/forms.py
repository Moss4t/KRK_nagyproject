...
class TopicMoveForm(forms.Form):
    ...
    forum = forms.ChoiceField(label=_('Válaszd ki hova szeretnéd áthelyezni'), widget=SelectWithDisabled)
    lock_topic = forms.BooleanField(label=_('Bejegyzés zárolása'), required=False)
    ...
    def clean_forum(self):
        ...
        if forum_id:
            ...
            if forum.is_category or forum.is_link or forum.id == self.topic.forum.id:
                raise forms.ValidationError('Nem választhatod ezt a témát az áthelyezéshez!')
