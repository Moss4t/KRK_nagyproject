...
class AbstractTopic(DatedModel):
    ...
    TYPE_CHOICES = (
        (TOPIC_POST, _('Normál bejegyzés')),
        (TOPIC_STICKY, _('Kiemelt bejegyzés')),
        (TOPIC_ANNOUNCE, _('Közlemény')),
    )
    type = models.PositiveSmallIntegerField(
        choices=TYPE_CHOICES, db_index=True, verbose_name=_('Bejegyzés tipusa'),
    )

    # A topic can be locked, unlocked or moved.
    TOPIC_UNLOCKED, TOPIC_LOCKED, TOPIC_MOVED = 0, 1, 2
    STATUS_CHOICES = (
        (TOPIC_UNLOCKED, _('Bejegyzés nyitva')),
        (TOPIC_LOCKED, _('Bejegyzés lezárva')),
        (TOPIC_MOVED, _('Bejegyzés áthelyezve')),
    )
    status = models.PositiveIntegerField(
        choices=STATUS_CHOICES, db_index=True, verbose_name=_('Bejegyzés státusza'),
    )
    ...
    approved = models.BooleanField(default=True, db_index=True, verbose_name=_('Engedélyezve'))

    # The number of posts included in this topic (only those that are approved).
    posts_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Hozzászólások száma'),
    )

    # The number of time the topic has been viewed.
    views_count = models.PositiveIntegerField(
        editable=False, blank=True, default=0, verbose_name=_('Megtekintések'),
    )

    # The date of the latest post.
    last_post_on = models.DateTimeField(blank=True, null=True, verbose_name=_('Utoljára hozzászólva'))
    ...
    first_post = models.ForeignKey(
        ...
        on_delete=models.SET_NULL, verbose_name=_('Első hozzászóló'),
    )
    last_post = models.ForeignKey(
        'forum_conversation.Post', editable=False, related_name='+', blank=True, null=True,
        on_delete=models.SET_NULL, verbose_name=_('Utolsó hozzászóló'),
    )

    # Many users can subscribe to this topic
    subscribers = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name='topic_subscriptions', blank=True,
        verbose_name=_('Feliratkozottak'),
    )
...
class AbstractPost(DatedModel):
    ...
    subject = models.CharField(verbose_name=_('Tárgy'), max_length=255)
    ...
    enable_signature = models.BooleanField(
        default=True, db_index=True, verbose_name=_('Aláirásod hozzácsatolása'),
    )
