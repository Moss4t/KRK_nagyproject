...
class TopicPollOptionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['text'].widget.attrs['placeholder'] = _('Ide ird a szavazási opciókat')
...
class BaseTopicPollOptionFormset(BaseModelFormSet):
    ...
    def clean(self):
        ...
        if number_of_options < 2:
            raise forms.ValidationError('2 szavazási opciónak lennie kell.')
...
class TopicPollVoteForm(forms.Form):
    ...
    def clean(self):
        ...
        if 'options' not in cleaned_data:
            msg = _('Ki kell egyet választanod a szavazáshoz.')
            raise ValidationError(msg)

        options = cleaned_data['options']
        if len(options) > self.poll.max_options:
            msg = _('Túl sok dologra próbáltál szavazni.')
            raise ValidationError(msg)
